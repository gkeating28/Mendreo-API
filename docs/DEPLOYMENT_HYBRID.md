...
| `FUNCTION_INVOCATION_FAILED` / `No module named 'mendreo.settings'` | Redeploy latest commit. Vercel loads `mendreo/mendreo/wsgi.py` from repo root, so `mendreo/` (outer) shadows the Django package. Fix: `mendreo/settings.py` and `mendreo/urls.py` shims re-export the inner modules. Do **not** set `PYTHONPATH=mendreo`. |
