...
- [ ] `GET/PATCH` consumer knowledge profile (grouped by category, current value, source, confidence, updated_at)
- [ ] Entry history + activity feed (filter by source)
- [ ] Admin edit value → new entry (`source=admin`)
- [ ] Runtime service: `get_current_knowledge_summary(consumer)` for session start
- [ ] Runtime write path: create entries from chat / extraction (`source=question` or `ai`)
- [ ] Wire summary into Agent session context (read path)
...
| 7 | Knowledge field value types (text/number/enum/multi)? | §1.3 | _TBD_ | |
